...
resourcename="sqoin"  # lower case, only
resourcegroup="sqoin_rg"
...
# git clone https://github.com/apilogicserver/sqoin.git
# cd sqoin
...
az group create --name sqoin_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group sqoin_rg --sku S1 --is-linux
az container create --resource-group sqoin_rg --name sqoin --image apilogicserver/sqoin:latest --dns-name-label sqoin --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//sqoin.westus.azurecontainer.io:5656'
